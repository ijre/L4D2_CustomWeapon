...
### Issues:
- Currently, in order to make the viewmodel and HUD icons fully functional for a client it REQUIRES them manually acquiring the "weapon_csbase_gun.txt" file, which can be done via a workshop mod or just downloading it directly. This is important to note because the client in L4D2 refuses to download anything from `scripts/` when joining a server.  
